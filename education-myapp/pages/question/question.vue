...
import {getHotquestionList,getNewquestionList,getWaitquestionList} from '@/api/api.js'
...
export default {
 ...
 const changeindex = (type,index)=>{
	 ...
	 if(data.activeIndex == 0){
		 ...
		 getNewquestionList({current:1, size: 10}).then((res)=>{
		 	 data.hotQuestionList = res.data.data.records
			  data.isshow = false
		 })
	 }else{
		 getWaitquestionList({current:1, size: 10}).then((res)=>{
		 	 data.hotQuestionList = res.data.data.records
			  data.isshow = false
		 })
	 }
		...
		console.log(1);
  		if(this.hotQuestionList.length > 40){
			...
			if(this.activeIndex == 0){
				getHotquestionList({current:this.current, size: 10}).then((res)=>{
					 this.hotQuestionList = [...this.hotQuestionList,...res.data.data.records] 
				})
			}else if(this.activeIndex == 1){
				console.log(1);
				getNewquestionList({current:this.current, size: 10}).then((res)=>{
					 this.hotQuestionList = [...this.hotQuestionList,...res.data.data.records] 
				})
			}else{
				getWaitquestionList({current:this.current, size: 10}).then((res)=>{
					 this.hotQuestionList = [...this.hotQuestionList,...res.data.data.records] 
				})
			}
  		}
