<template>
	<view>
		<scroll-view scroll-X="true" class="top-box">
			<view :class=" topindex == index? ' active top-box-item' :'top-box-item' " 
			v-for="item,index in topList" :key="item.id" @click="changeindex(item,index)"> 
			{{item.name}} 
			</view>
		</scroll-view>
		<view class="content">
...
import { getHotrecom,getReadList} from '@/api/api.js'
...
export default {
 ...
 const route = useRoute();
	...
	getReadList({current: 1,size: 10,categoryId:0}).then((res)=>{
		data.readList = res.data.data.records
	})
	const changeindex = (obj,index)=>{
		data.topindex = index
		data.categoryId = obj.id
		getReadList({current: 1,size: 10,categoryId:obj.id}).then((res)=>{
			data.readList = res.data.data.records
		})
	}
	const gotop = ()=>{
  	...
  	if(this.status){
  		...
  		if(this.readList.length > 40){
  			return this.isshow = true
  				...
  				getReadList({current:this.current,size:10,categoryId:this.categoryId}).then((res)=>{
